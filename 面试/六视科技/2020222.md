    ...
    public List<UserAnswerRecord> getUserAnswerRecords(List<AnswerRecord> records) {
        ...
        if (records != null && records.size() > 0) {
            ...
            for (int i = 0; i < records.size(); i++) {
                ...
                // 这道习题是否做过
                if (records.get(i).last == -1) {
                    mComputed++;
                }

                //树状结构和知识点ID
                String parents = records.get(i).parents;
                String[] split = parents.split(",");
                //装载树状结构id
                parentsId[i] = Long.parseLong(split[0]);


                long[] courseNodeId = new long[split.length-1];
                for (int j=1;j<split.length;j++){
                    courseNodeId[j-1] = Long.parseLong(split[j]);
                }

                List<CourseNode> courseNodes = getCourseNodes(courseNodeId);

                List<UserAnswerRecord> userAnswerRecord = getUserAnswerRecord(parentsId);
                for (UserAnswerRecord record : userAnswerRecord) {
                    record.completed += mComputed;
                    record.correct += mCorrect;
                    record.master+= courseNodes.size();
                }
                mUserAnswerRecord.addAll(userAnswerRecord);
            }

        }

        return mUserAnswerRecord;
    }
