...
我们客户端向服务器请求资源时一件成本较高的事情，因为链路较长、网络不可控；

我们的需要把这个获取到的数据缓存起来啊，下次请求时尽可能使用缓存的数据；
...
服务器中使用Cache-Control头字段来标记资源的有效期，值【max-age=30】代表这个资源只能缓存30秒，之后就给过期不能使用了；
...
当浏览器中使用【Cache-Control】做缓存控制都是向服务器请求最新数据刷数据，不能很好的使用缓存数据；并且缓存数据还会失效，使用前还必须去服务器验证是否失效；

###### 两个连续的请求组成的【验证动作】

- 先时一个HEAD，获取当前资源修改的时间的细信息，与缓存数据进行对比，没有变化就使用缓存，否则就发送请求获取最新的数据；
- 两次请求的网络成本比较高，HTTP协议定义了一些列【If】开头的【条件请求】字段，专门用来验证资源是否过期，把上述两步做的事情合并成一步；
